...
class Notifier {
  ...
  async sendBark(title = '', content = '', { link, key }) {
    const pushKey = key || this.barkKey
   ...
   * @return {Promise<String[]>}   推送结果
   */
  async notify(title, content, options = {}) {
    const promise = []

    if (this.barkKey) {
      promise.push(this.sendBark(title, content, options))
    }

    if (this.telegram && this.telegram.botToken) {
      promise.push(this.sendTelegram(title, content, options))
    }

    if (this.workWechat) {
      promise.push(this.sendWorkWechat(title, content, options))
    }

    if (this.serverChanToken) {
      promise.push(this.sendServerChan(title, content, options))
    }
